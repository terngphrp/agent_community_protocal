...
SKILL_SOURCE_DIR="$PROJECT_ROOT/skills/a2a-cross-session"
...
install_skill_dir() {
  local root="$1"
  local label="$2"
  local target="$root/a2a-cross-session"

  mkdir -p "$target"
  cp -R "$SKILL_SOURCE_DIR/"* "$target/"
  chmod +x "$target/scripts/a2a_cross_session.py"
  echo "Installed $label skill: $target"
}

echo "=== Installing a2a-cross-session user skill ==="
...
if [[ "$INSTALL_CLI" == true ]]; then
  ...
  CLI_WRAPPER="$HOME/.local/bin/a2a-cross-session"
  cat > "$CLI_WRAPPER" << EOF
#!/usr/bin/env bash
set -euo pipefail
...
cd "\$A2A_LOCAL_ROOT"
exec \$PYTHON_BIN "\$A2A_LOCAL_ROOT/skills/a2a-cross-session/scripts/a2a_cross_session.py" "\$@"
EOF
  chmod +x "$CLI_WRAPPER"
